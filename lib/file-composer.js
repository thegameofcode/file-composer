...
function searchIncludes(include, fileName, contents) {
	var search;
	...
	while ((search = includePattern.exec(contents)) !== null) {
		try {
			...
			var line = findLineNumber(contents, search[0]);
			throw new Error('Malformed include: ' + search[0] + ' at line ' + line.number + ' of ' + fileName + '[' + line.text + ']' + ' Error: ' + e);
		}
...
function addFile(parent, fileName, options) {
	...
	if (!include) {
		...
		searchIncludes(include, fileName, input);
	}
	if (options) {
...
function replacePattern(string, pattern, value) {
	...
	while ((search = pattern.exec(result)) !== null) {
		var replacement = value !== undefined ? value : search[1];
...
function replaceDefaultValues(include) {
	...
	result = replacePattern(result, /{{.*?}}/g, ''); //clean non passed values
...
function replaceValues(string, values) {
	...
	if (options.v) {
		...
		for (i = 0; i < argValues.length; i++) {
			...
			result = replacePattern(result, new RegExp('{{\\s*' + key + '\\s*[^}]*}}', 'g'), value);
		}
	}

	if (values) {
		for (var key in values) {
			result = replacePattern(result, new RegExp('{{\\s*' + key + '\\s*[^}]*}}', 'g'), '{{' + key + '||' + values[key] + '}}');
		}
	}
